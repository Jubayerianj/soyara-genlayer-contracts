...
//
//   1. VALIDATE — `validate_proposal` is a GenLayer consensus WRITE, so it needs
//      a funded GenLayer account to submit the transaction.
//   2. SETTLE   — `AgentExecutor.approveTradeWithParams` and the `execute*`
//      functions carry an `onlyAgent` modifier, so the caller must be an
//      authorised agent on the executor.
//
