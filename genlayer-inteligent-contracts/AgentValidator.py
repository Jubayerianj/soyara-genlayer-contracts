...
class AgentValidator(gl.Contract):
    ...
    def _parse_hex(self, value: str) -> bytes:
        ...
        if raw.startswith("0x") or raw.startswith("0X"):
            raw = raw[2:]
        if len(raw) % 2 != 0:
            # gl.vm.UserError rather than a bare ValueError: genvm-lint warns on
            # raw Python exceptions in contract code because they do not carry a
            # caller-facing message across the VM boundary.
            raise gl.vm.UserError("hex string has an odd number of characters")
